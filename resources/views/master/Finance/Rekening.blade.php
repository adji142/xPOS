						...
						<div class="card card-custom gutter-b bg-transparent shadow-none border-0" >
							<div class="card-header align-items-center  border-bottom-dark px-0">
								<div class="card-title mb-0">
									<h3 class="card-label mb-0 font-weight-bold text-body">Rekening 
									</h3>
								</div>
							    <div class="icons d-flex">
									<a href="{{ url('rekening/form/-') }}" class="btn btn-outline-primary rounded-pill font-weight-bold me-1 mb-1">Tambah Data</a>
							...
							<div class="card-body" >
								<div class="table-responsive" id="printableTable">
									<table id="orderTable" class="display" style="width:100%">
										...
										<tbody>
											@if (count($rekening) > 0)
												@foreach($rekening as $v)
												<tr>
													<td>{{ $v['KodeRekening'] }}</td>
													<td>{{ $v['NamaRekening'] }}</td>
													<td>{{ $v['NamaKelompok'] }}</td>
													<td>{{ $v['Level'] }}</td>
													<td>{{ $v['JenisRekening'] }}</td>
													<td>{{ $v['KodeRekeningInduk'] }}</td>
													<td>{{ $v['SaldoBase'] }}</td>
													<td>
														<div class="card-toolbar text-end">
															...
															<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdowneditButton1"  style="position: absolute; transform: translate3d(1001px, 111px, 0px); top: 0px; left: 0px; will-change: transform;">
																<a class="dropdown-item" href="{{ url('rekening/form/' . $v['KodeRekening']) }}">Edit</a>
																<a class="dropdown-item" title="Delete" href="{{ route('rekening-delete', $v['KodeRekening']) }}" data-confirm-delete="true">Delete</a>
															</div>
														</div>
													</td>
												</tr>
...
<script type="text/javascript">
	jQuery(document).ready(function() {
		jQuery('#KelompokRekening').select2();
