...
<body>
    <header>
        <img src="{{ $data[0]['icon'] }}" alt="Logo" width="50%" id="logo">
        <center>
            <h3>{{ $data[0]['NamaPartner'] }}</h3>
            <p>{{ $data[0]['AlamatTagihan'] }}</p>
            <p>{{ $data[0]['NoTlp'] }}</p>
        </center>
        <hr>
    </header>
    <p>TRX : {{ $data[0]['NoTransaksi'] }}</p>
    <table class="bill-details">
        <tbody>
            <tr>
                <td>Date : <span>{{ $data[0]['TglTransaksi'] }}</span></td>
            </tr>
            <tr>
                <td>Cashier #: Default</span></td>
    ...
    <table class="items">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="line price">{{ number_format($data[0]['Total']) }}</td>
            </tr>
            ...
            <tr>
                ...
                <td class="price">{{ number_format($data[0]['Pajak']) }}</td>
            </tr>
            <tr>
                <td colspan="3" class="sum-up"><strong>Total</strong></td>
                <td class="price">{{ number_format($data[0]['Total']) }}</td>
            </tr>
            @if ($v['Diskon'] > 0)
                <tr>
                    <td colspan="3" class="sum-up">Anda Hemat</td>
                    <td class="price">{{ number_format($data[0]['Diskon']) }}</td>
                </tr>
            @endif
            <tr>
                <td colspan="3" class="sum-up"><strong>Bayar</strong></td>
                <td class="price">{{ number_format($data[0]['TotalPembayaran']) }}</td>
            </tr>
            <tr>
                <td colspan="3" class="sum-up"><strong>Kembali</strong></td>
                <td class="price">{{ number_format($data[0]['Kembalian']) }}</td>
            </tr>
        </tbody>
    </table>
    <section>
        <p>
            Bayar Dengan : <span>{{ $data[0]["NamaMetodePembayaran"] }}</span><br>
            Reff : {{ $data[0]["ReffPembayaran"] }}
        </p>
        <hr>
