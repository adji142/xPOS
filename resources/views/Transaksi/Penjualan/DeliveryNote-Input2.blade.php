                            	...
                            	<div class="form-group row">
                                    ...
                                    <div class="col-md-6">
                            			...
                            			<div id="Keterangan" style="height: 100px;">
                                            {!! count($deliveryheader) > 0 ? $deliveryheader[0]['Keterangan'] : '' !!}
                                        </div>
                            		</div>
                                    ...
                                    <div class="col-md-6">
                            			...
                            			<div id="SyaratDanKetentuan" style="height: 100px;">
                                            {!! count($deliveryheader) > 0 ? $deliveryheader[0]['SyaratDanKetentuan'] : '' !!}
                                        </div>
                            		</div>
                            		...
                            		<div class="col-md-5">
                            			<table>
                            				...
                            				<tr>
                            					<td>Total</td>
                            					<td>:</td>
                            					<td><input type="text" align="right" name="TotalPenjualan" id="TotalPenjualan" readonly="" class="form-control aligned-textbox" value="{{ count($deliveryheader) > 0 ? $deliveryheader[0]['TotalPembelian'] : '0' }}"></td>
                            				</tr>
                            			</table>
                            		</div>
...
<script>
    ...
    $(document).ready(function () {
        ...
        if (jQuery('#formtype').val() == "edit") {
            jQuery('#KodePelanggan').val(deliveryheader[0]["KodePelanggan"]).trigger('change');
            ...
            deliverydetail.forEach(function(dlv, index) {
                console.log(dlv);
                const newRow = `
                    <tr>
                        <td>
    ...
    jQuery('#KodePelanggan').change(function () {
        console.log("filteredOrderDetail");
        $.ajax({
            async:false,
            type: 'post',
    ...
    jQuery('#btSave').on('click', function(){
        ...
        jQuery('#tableDetailItem tbody tr').each(function() {
            ...
            const qtyorder = parseFloat(row.find('.qtyorder').val()) || 0;
            const qtykirim = parseFloat(row.find('.qtykirim').val()) || 0;
            ...
            const rowData = {
                ...
                'QtyOrder' : qtyorder,
                'QtyKirim' : qtykirim * row.find('.select-item option:selected').data('qtykonversi'),
                        ...
                        if (v.OutStanding > 0) {
                            const newRow = `
                                <tr>
                                    ...
                                    <td><input type="number" class="form-control qtyorder" value="${v.Qty}" readonly/></td>
                                    <td><input type="number" class="form-control qtykirim" value="0" min=1 max="${v.Qty}"/></td>
                                    <td>
                                        <select class="form-control select2 select-gudang" name="gudang[]">
                                            <option value="">Pilih Gudang</option>
                                            @foreach($gudang as $gd)
    ...
    function calculateRowTotal(row) {
        let qty = parseFloat(jQuery(row).find('.qtykirim').val()) || parseFloat(jQuery(row).find('.qtyorder').val());
    ...
    function updateSummary() {
        ...
        jQuery('#tableDetailItem tbody tr').each(function () {
            let qty = parseFloat(jQuery(this).find('.qtykirim').val()) || parseFloat(jQuery(this).find('.qtyorder').val());
