	...
	<div class="container-fluid">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb bg-white mb-0 px-0 py-2">
				<li class="breadcrumb-item active" aria-current="page">Stock Opname History</li>
			</ol>
		</nav>
	</div>
</div>
						...
						<div class="card card-custom gutter-b bg-transparent shadow-none border-0" >
							<div class="card-header align-items-center  border-bottom-dark px-0">
								<div class="card-title mb-0">
									<h3 class="card-label mb-0 font-weight-bold text-body">Stock Opname 
									</h3>
								</div>
							    <div class="icons d-flex">
									<a href="{{ url('stockopname/form/-') }}" class="btn btn-outline-primary rounded-pill font-weight-bold me-1 mb-1">Tambah Data</a>
									<div class="icons d-flex">
								</div>
								</div>
							</div>
							...
							<div class="card-body" >
								<div class="table-responsive" id="printableTable">
									<table id="orderTable" class="display" style="width:100%">
										...
										<tbody>
											@if (count($stockopname) > 0)
												@foreach($stockopname as $v)
												<tr>
													<td>{{ $v['NoTransaksi'] }}</td>
													<td>{{ $v['TglTransaksi'] }}</td>
													<td>{{ number_format($v['JumlahItem']) }}</td>
													<td>{{ number_format($v['QtyOpname']) }}</td>
                                                    <td>{{ number_format($v['Stock']) }}</td>
                                                    <td>{{ number_format( $v['QtyOpname'] - $v['Stock'] ) }}</td>
													{{-- <td>
														<div class="card-toolbar text-end">
															...
															<div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdowneditButton1"  style="position: absolute; transform: translate3d(1001px, 111px, 0px); top: 0px; left: 0px; will-change: transform;">
																<a class="dropdown-item" href="{{ url('stockopname/form/' . $v['NoTransaksi']) }}">Edit</a>
															</div>
														</div>
													</td> --}}
												</tr>
