		...
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb bg-white mb-0 px-0 py-2">
				<li class="breadcrumb-item active" aria-current="page">
					<a href="{{route('gr')}}">Daftar Pengakuan Barang</a>
				</li>
				<li class="breadcrumb-item active" aria-current="page">Pengakuan Barang</li>
			</ol>
		</nav>
	</div>
</div>
							...
							<div class="card-header align-items-center  border-bottom-dark px-0">
								<div class="card-title mb-0">
									<h3 class="card-label mb-0 font-weight-bold text-body">
										@if (count($pengakuanheader) > 0)
											<input type="hidden" name="formtype" id="formtype" value="edit">
                                    		Edit Pengakuan Barang
	                                	@else
	                                		<input type="hidden" name="formtype" id="formtype" value="add">
	                                    	Tambah Pengakuan Barang
	                                	@endif
									</h3>
								</div>
							</div>
                            	...
                            	<div class="form-group row">
                            		<div class="col-md-4">
                            			...
                            			<fieldset class="form-group mb-3">
                            				<input type="text" class="form-control" id="NoTransaksi" name="NoTransaksi" placeholder="<Auto>" value="{{ count($pengakuanheader) > 0 ? $pengakuanheader[0]['NoTransaksi'] : '' }}" required="">
                            			</fieldset>
                            		...
                            		<div class="col-md-4">
                            			...
                            			<fieldset class="form-group mb-3">
                            				<select name="Status" id="Status" class="js-example-basic-single js-states form-control bg-transparent" >
												<option value="O" {{ count($pengakuanheader) > 0 ? $pengakuanheader[0]['Status'] == 'O' ? "selected" : '' :""}} >OPEN</option>
												<option disabled="" value="C" {{ count($pengakuanheader) > 0 ? $pengakuanheader[0]['Status'] == 'C' ? "selected" : '' :""}} >CLOSE</option>
												<option value="D" {{ count($pengakuanheader) > 0 ? $pengakuanheader[0]['Status'] == 'D' ? "selected" : '' :""}}>CANCEL</option>
                            		...
                            		<div class="col-md-4">
                            			...
                            			<fieldset class="form-group mb-3">
                            				<input type="date" class="form-control" id="TglTransaksi" name="TglTransaksi" placeholder="<Auto>" value="{{ count($pengakuanheader) > 0 ? $pengakuanheader[0]['TglTransaksi'] : '' }}" required="">
                            			</fieldset>
                            		</div>
                            		...
                            		<div class="col-md-6">
                            			...
                            			<fieldset class="form-group mb-3">
                            				<input type="text" class="form-control" id="NoReff" name="NoReff" placeholder="Masukan No Reff" value="{{ count($pengakuanheader) > 0 ? $pengakuanheader[0]['NoReff'] : '' }}" >
                            			</fieldset>
                            		</div>
                            		...
                            		<div class="col-md-6">
                            			...
                            			<fieldset class="form-group mb-3">
                            				<input type="text" class="form-control" id="Keterangan" name="Keterangan" placeholder="Masukan Keterangan" value="{{ count($pengakuanheader) > 0 ? $pengakuanheader[0]['Keterangan'] : '' }}" >
                            			</fieldset>
                            		</div>
                            		...
                            		<div class="col-md-5">
                            			<table>
                            				<tr>
                            					<td>Total</td>
                            					<td>:</td>
                            					<td><input type="text" align="right" name="TotalTransaksi" id="TotalTransaksi" readonly="" class="form-control aligned-textbox" value="{{ count($pengakuanheader) > 0 ? $pengakuanheader[0]['TotalTransaksi'] : '0' }}"></td>
                            				</tr>
                            			</table>
                            		</div>
...
<script type="text/javascript">
	...
	jQuery(function () {
		jQuery(document).ready(function() {
	    	...
	    	pengakuanHeader = <?php echo json_encode($pengakuanheader); ?>;
	    	pengakuanDetail = <?php echo json_encode($pengakuandetail); ?>;

	    	oItemMaster = <?php echo $item ?>;

	    	console.log(pengakuanHeader)
			if (jQuery('#formtype').val() == "edit") {
				formatCurrency(jQuery('#TotalTransaksi'), pengakuanHeader[0]["TotalTransaksi"]);
	      		StatusTransaksi = pengakuanHeader[0]["Status"];
	      		...
	      		BindGridDetail(<?php echo json_encode($pengakuandetail) ?>);
			}
			else{
				BindGridDetail([])	
			}
