	...
	<div class="container-fluid">
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb bg-white mb-0 px-0 py-2">
				<li class="breadcrumb-item active" aria-current="page">Pembayaran Pembelian Konsinyasi</li>
			</ol>
		</nav>
	</div>
</div>
						...
						<div class="card card-custom gutter-b bg-transparent shadow-none border-0" >
							<div class="card-header align-items-center  border-bottom-dark px-0">
								<div class="card-title mb-0">
									<h3 class="card-label mb-0 font-weight-bold text-body">Pembayaran Pembelian Konsinyasi 
									</h3>
								</div>
							    <div class="icons d-flex">
									<a href="{{ url('pembayaranpembeliankonsinyasi/form/-') }}" class="btn btn-outline-primary rounded-pill font-weight-bold me-1 mb-1">Tambah Data</a>
...
<script type="text/javascript">
    ...
    function GetHeader() {
        $.ajax({
            async:false,
            type: 'post',
            url: "{{route('pembayaranpembeliankonsinyasi-readheader')}}",
            headers: {
    ...
    function GetDetail(NoTransaksi) {
        $.ajax({
            async:false,
            type: 'post',
            url: "{{route('pembayaranpembeliankonsinyasi-readdetail')}}",
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}' // Include the CSRF token in the headers
            },
            data: {
            ...
            columns: [
                ...
                },
                {
                    ...
                    cellTemplate: function(cellElement, cellInfo) {
                        var link = "pembayaranpembeliankonsinyasi/form/"+cellInfo.data.NoTransaksi;
