		...
		<nav aria-label="breadcrumb">
			<ol class="breadcrumb bg-white mb-0 px-0 py-2">
				<li class="breadcrumb-item active" aria-current="page">
					<a href="{{route('opembelian')}}">Daftar Order Pembelian</a>
				</li>
				<li class="breadcrumb-item active" aria-current="page">Order Pembelian</li>
			</ol>
		</nav>
	</div>
</div>
                            				...
                            				<select name="KodeSupplier" id="KodeSupplier" class="js-example-basic-single js-states form-control bg-transparent" >
												...
												@foreach($supplier as $ko)
													<option 
                                                        ...
                                                        {{ count($orderheader) > 0 ? $orderheader[0]['KodeSupplier'] == $ko->KodeSupplier ? "selected" : '' :""}}
                                                    >
                                                        {{ $ko->NamaSupplier }}
                                                    </option>
												@endforeach
                            		...
                            		<div class="col-md-5">
                            			<table>
                            				...
                            				<tr>
                            					<td>Total</td>
                            					<td>:</td>
                            					<td><input type="text" align="right" name="TotalPembelian" id="TotalPembelian" readonly="" class="form-control aligned-textbox" value="{{ count($orderheader) > 0 ? $orderheader[0]['TotalPembelian'] : '0' }}"></td>
                            				</tr>
                            			</table>
                            		</div>
...
<script type="text/javascript">
	...
	jQuery(function () {
		jQuery(document).ready(function() {
	    	...
	    	console.log(<?php echo $orderdetail ?>)
			...
			if (jQuery('#formtype').val() == "edit") {
				formatCurrency(jQuery('#TotalTransaksi'), orderHeader[0]["TotalTransaksi"]);
	      		...
	      		BindGridDetail(<?php echo $orderdetail ?>);
			}
			else{
				BindGridDetail([])	
			}
		...
		jQuery('#btSave').click(function () {
			...
			if (jQuery('#formtype').val() == "add") {
				$.ajax({
					url: "{{route('opembelian-storeJson')}}",
					type: 'POST',
		            ...
		            success: function(response) {
		            	if (response.success == true) {
		            		Swal.fire({
		                        ...
		                        // location.reload();
		                        window.location.href = '{{url("opembelian")}}';
		                    });
		            	}
		            	else{
		            		Swal.fire({
